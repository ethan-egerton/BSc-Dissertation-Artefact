...
namespace midism.classes
{
    ...
    public class DeviceHandler()
    {
        ...
        private void OnEventReceived(object sender, MidiEventReceivedEventArgs e)
        {
            ...
            GlobalVariables.MIDIactivation = activation;

            foreach (KeyValuePair<int, List<String>> pair in GlobalVariables.translations)
            {
                System.Diagnostics.Debug.WriteLine(string.Join(", ", pair.Value));
                ...
                if (type == "Windows Parameter")
                {
                    if (value == "Volume")
                    {
                        double volume = ((double)activation / 127) * 100;
                        System.Diagnostics.Debug.WriteLine(activation);
                        System.Diagnostics.Debug.WriteLine(volume);
                        SystemAudio.WindowsSystemAudio.SetVolume((int)volume);
                    }
                }
            }
        }
